...
http://yourGeoJsonSearchAddress
*/
;
(function ($) {

    var options = {
        geojsonServiceAddress: "https://aichii1.github.io/curly-disco/example/stationer.geojson",
    ...
    function drawGeoJsonList() {
        ...
        m.fitBounds(searchLayer.getBounds());
    }

    function focusGeoJson(index) {

        if (features[index].geometry.type === "Point" && options.pointGeometryZoomLevel !== -1) {
            m.setView([features[index].geometry.coordinates[1], features[index].geometry.coordinates[0]], options.pointGeometryZoomLevel);
        }
        else {
            m.fitBounds(getBoundsOfGeoJsonObject(features[index].geometry));
        }
